 ...
 return(
   <div  className="App">
     <div style={{display:"flex",justifyContent:"space-between"}}>
      <h1 className="title">RV's MovieTreat❤️</h1>
       

    <Box sx={{padding:"1rem",display:"flex"}}>
      {(show)?<Button variant="contained" color="error" onClick={()=>setShow(!show)}>-</Button>:<Button variant="contained" color="error" onClick={()=>setShow(!show)}>Add Movie</Button>}
      <div className="addMovie" style={style}>
       <Box component="form" noValidate sx={{ display: 'grid', gridTemplateColumns:"1fr", gap: 0.5, padding:"0.5rem" }}>   
        ...
        <Button variant="contained" color="error" onClick={()=>{setShow(false);adding([...added])}}>Cancel</Button>
   ...
   <Box sx={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(15rem,1fr))",gap:"2rem;",border:"2px solid red",padding:"1rem",borderRadius:"15px",background:"#bea3a3"}}>
      {added.map(({name,image,rating,summary})=><First key={name} name={name} image={image} rating={rating} summary={summary}/>)}
   </Box>
